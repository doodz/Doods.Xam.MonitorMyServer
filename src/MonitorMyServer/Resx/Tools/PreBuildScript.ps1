...
Get-Content $SolutionDir | where { $_ -match "Project.+, ""(.+csproj)""," } | foreach { $matches[1] } | % {Get-Content $SolutionDir$_ |  where { $_ -match "<PackageReference Include=.+Version="} } | Sort-Object -Unique | Out-File "$ProjectDir\Resx\ThirdPartyLicenseOverview_doods.txt"
...
# save
#powershell.exe -ExecutionPolicy Bypass -File $(ProjectDir)\Resx\Tools\PreBuildScript.ps1  $(ProjectDir) $(SolutionDir) $(TargetDir) $(SolutionPath)
#$(ProjectDir) C:\GitHub\Doods.Xam.MonitorMyServer\src\MonitorMyServer
#*$(SolutionDir) C:\GitHub\Doods.Xam.MonitorMyServer
#$(TargetDir) C:\GitHub\Doods.Xam.MonitorMyServer\src\MonitorMyServer\bin\Release\netstandard2.0
#*$(SolutionPath) C:\GitHub\Doods.Xam.MonitorMyServer\Doods.Xam.MonitorMyServer.sln

...
namespace Doods.Xam.MonitorMyServer.Views.Linux.Logs
{
    public class LogLineModel
    {
        ...
        public LogLineModel(string logline, long rownum)
        {
            ...
            if (m.Success)
            {
                ...
                DateTime dateValue;
                if (DateTime.TryParseExact(m.Groups[1].Value, Pattern, CultureInfo.InvariantCulture , DateTimeStyles.None, out dateValue))
                {
                    Date = dateValue;
                    Ts = Date.Ticks;
                }
    ...
    public class LogsViewmodel : ViewModelWhithState
    {
        ...
        private async Task OnChanged()
        {
            if (SelectedLogFile != null)
                try
                {
                    ...
                    foreach (var g in gp)
                    {
                        g.Skip(1).ForEach(l=> l.IsNotIn = false);
                    }
        ...
        private void TologLine(IEnumerable<string> logs)
        {
